...
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/router/security_wrapper.py" \
...
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/router/read_cortex_sessions.py" --limit 3
```
...
2. Review routing script logic in `${CLAUDE_PLUGIN_ROOT}/scripts/router/route_request.py`
...
To customize routing beyond default logic, edit `${CLAUDE_PLUGIN_ROOT}/scripts/router/route_request.py`:
